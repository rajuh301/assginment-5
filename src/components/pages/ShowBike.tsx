...
import { useCreateRentalsMutation, useGetAllBikeQuery } from '../../redux/features/bikes/bikeApi';
import { Bike } from '../types/bike.type';
import { FaCalendarAlt, FaCheckCircle, FaTimesCircle, FaDollarSign, FaInfoCircle, FaCogs, FaIndustry } from 'react-icons/fa';

const ShowBike: React.FC = () => {
  const { data, isLoading, isFetching, error } = useGetAllBikeQuery(undefined);
  const [createRentals, { isLoading: isRenting, error: rentalError }] = useCreateRentalsMutation();

  if (isLoading || isFetching) {
    return <p className="text-center text-gray-600">Loading bikes...</p>;
  }

  if (error) {
    return <p className="text-center text-red-600">Failed to load bikes. Please try again later.</p>;
  }

  const handleSubmit = async (id: string) => {
    const startTime = new Date().toISOString();
    try {
      await createRentals({ bikeId: id, startTime }).unwrap();
        ...
        {data?.data?.map((bike: Bike) => (
          <div key={bike._id} className="relative bg-white shadow-lg rounded-lg overflow-hidden transform hover:scale-105 transition-transform duration-300">
            ...
            onClick={() => (document.getElementById('my_modal_3') as HTMLDialogElement)?.showModal()}
                ...
                className={`py-2 px-4 font-semibold rounded-lg focus:outline-none transition-colors duration-300 ${bike.isAvailable
            ...
            <dialog id="my_modal_3" className="modal">
              <div className="modal-box">
                ...
                {rentalError && <p className="text-red-600">Failed to rent the bike. Please try again.</p>}
              </div>
            </dialog>
          </div>
        ))}
