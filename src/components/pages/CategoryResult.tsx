...
import {  FaCogs, FaInfoCircle, FaCalendarAlt, FaCheckCircle, FaTimesCircle, FaDollarSign, FaIndustry } from "react-icons/fa";
import Navbar from "./Navbar"; 
import { useGetBikeByCategoryQuery } from "../../redux/features/bikes/bikeApi";
import { Bike } from "../types/bike.type";

const CategoryResult = () => {
  const location = useLocation();
  const category = new URLSearchParams(location.search).get("category") || "";
  const { data,  } = useGetBikeByCategoryQuery(category);

  useEffect(() => {
    // Optionally, track analytics or reset scroll position
    window.scrollTo(0, 0);
  }, [category]);

  function handleSubmit(_id: any): void {
